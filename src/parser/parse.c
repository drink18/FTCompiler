...
static Ast_Stm_t Parse_assignment ()
{
 ...
 // lval = Parse_lval (firstId)
  ...
  for (;;){
        printf("[[%d]],%s\n",current->kind,get_tok_str(current->kind,NULL));
    switch (current->kind){
    case TOKEN_DOT:
      advance ();
      firstId = eatToken(TOKEN_ID);
   ...
   switch(tok)
   	{
		...
		break;
		default:
		    if(Hash_lookup(tyTable,get_tok_str(tok,NULL)))
            {
                return 1;
            }
			if(*atr==VT_STATIC)
...
static LinkedList_t Parse_declarationList(int flag)
{
  ...
  for (;;){
  	static_set=0;
	if(!Parse_type(&static_set)) return list;
	type=current;
	id1 = convertToken (type);
	advance();
	VariableNameStart:
    name = eatToken (TOKEN_ID);
    id2 = convertToken (name);
    printf("[%d]%s  [%d]%s\n",type->kind,get_tok_str(type->kind,NULL),name->kind,get_tok_str(name->kind,NULL));
    ...
    printf("init->kind = %d\n",init->kind);
	if(flag&&init->kind!=AST_EXP_INTLIT&&init->kind!=AST_EXP_DOUBLELIT)
    {
        //     tcc_error ("global variable init must use constant");
    }
